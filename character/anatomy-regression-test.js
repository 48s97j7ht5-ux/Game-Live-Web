...
const geometryValidation=api.validateGeometry?.();if(geometryValidation&&!geometryValidation.pass)throw new Error('Skeleton v1.5 geometry validation failed: '+JSON.stringify(geometryValidation.checks));
...
function diagnostics(name){const joints={};for(const n of important){try{const p=mechanics.getJointWorld(n);joints[n]=[p.x,p.y,p.z]}catch{}}const segments={};for(const n of api.segmentNames){const s=api.getSegment(n);if(s)segments[n]=s.length}return{name,mechanicsVersion:mechanics.mechanicsVersion??mechanics.constructor.name,skeletonVersion:api.skeletonVersion,joints,segments,geometry:api.getGeometryMetrics?.()??null,geometryValidation};}
