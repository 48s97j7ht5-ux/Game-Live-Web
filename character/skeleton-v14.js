...
/** Skeleton v1.4 — standalone anatomical development line. */
export const SKELETON_CONTRACT_VERSION=1;
export const SKELETON_VERSION='1.4';
...
['C7','C6','C5','C4','C3'].forEach((k,i)=>{const t=(i+1)/5;spineWorld[k]=new THREE.Vector3(0,c7+(c1-c7)*t*.62,smooth(-.020,-.012,t))});
...
wn(t7g,'manubrium',...man.toArray(),.012,MAT.frame);wn(t7g,'sternumBody',...stern.toArray(),.011,MAT.frame);wn(t7g,'xiphoid',...xiph.toArray(),.009,MAT.frame);wb(t7g,'manubrium_sternumBody',man,stern,.009,MAT.frame);wb(t7g,'sternumBody_xiphoid',stern,xiph,.007,MAT.frame);
...
function buildShoulderGirdle(side){
 const g=side==='L'?-1:1;
 const scW=new THREE.Vector3(g*.018,sy+.012,.035),acW=new THREE.Vector3(g*(sx-.012),sy+.012,-.024),ghW=new THREE.Vector3(g*sx,sy,0);
 ...
 const scSup=new THREE.Vector3(g*.061,spineWorld.T2.y+.004,-.116);
 const scMed=new THREE.Vector3(g*.064,spineWorld.T3.y-.010,-.118);
 const scInf=new THREE.Vector3(g*.078,spineWorld.T7.y-.010,-.103);
...
const head=joint('head',vj.C1,new THREE.Vector3(0,(skY-.055)-c1y,.024));
...
function rebuildRestPose({stature:newStature=stature}={}){if(Math.abs(newStature-stature)>.000001)throw new Error('Skeleton v1.4: stature rebuild after dynamic torso validation');resetPose();return getRestMetrics()}
...
metrics.innerHTML=`<h3>Диагностика v1.4</h3><div class="row"><span>Рост</span><span>1750 мм</span></div><div class="row"><span>Architecture</span><span>standalone dynamic</span></div><div class="row"><span>Contract</span><span>v1</span></div><div class="row"><span>Limbs</span><span>native L/R</span></div><div class="row"><span>Spine</span><span>25 native vertebral joints</span></div><div class="row"><span>Shoulders</span><span>SC → AC → scapula/ST → GH</span></div>`;
const title=document.querySelector('.info .title');if(title)title.textContent='Skeleton v1.4';const sub=document.querySelector('.info .sub');if(sub)sub.innerHTML='standalone dynamic body · contract v1<br>25-joint spine · thorax-constrained shoulder development';
