...
function buildScapulaQuaternion(model,pivot,upward,posterior,external,restOffset){const sign=model.sideSign,n=surfaceNormal(model,pivot),up=new THREE.Vector3(0,1,0),tangentX=new THREE.Vector3().crossVectors(up,n).normalize();if(sign<0)tangentX.negate();const tangentY=new THREE.Vector3().crossVectors(n,tangentX).normalize(),qBase=new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().makeBasis(tangentX,tangentY,n)),q=qBase.clone().multiply(new THREE.Quaternion().setFromAxisAngle(n,rad(upward))).multiply(new THREE.Quaternion().setFromAxisAngle(tangentX,rad(posterior))).multiply(new THREE.Quaternion().setFromAxisAngle(tangentY,rad(sign*external)));if(restOffset)q.multiply(restOffset);return {q,qBase}}
