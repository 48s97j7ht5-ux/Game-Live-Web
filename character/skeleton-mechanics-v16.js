...
function buildScapulaQuaternion(model,pivot,upward,posterior,external,restOffset){const sign=model.sideSign,n=surfaceNormal(model,pivot),up=new THREE.Vector3(0,1,0),tangentX=new THREE.Vector3().crossVectors(up,n).normalize();if(sign<0)tangentX.negate();const tangentY=new THREE.Vector3().crossVectors(n,tangentX).normalize(),qBase=new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().makeBasis(tangentX,tangentY,n)),localNormal=new THREE.Vector3(0,0,1),localLateral=new THREE.Vector3(1,0,0),localVertical=new THREE.Vector3(0,1,0),q=qBase.clone().multiply(new THREE.Quaternion().setFromAxisAngle(localNormal,rad(upward))).multiply(new THREE.Quaternion().setFromAxisAngle(localLateral,rad(posterior))).multiply(new THREE.Quaternion().setFromAxisAngle(localVertical,rad(sign*external)));if(restOffset)q.multiply(restOffset);return {q,qBase}}
function solveScapula(api,side,elevation,planeShare){const scap=api.getJoint(`scapula_${side}`),sc=api.getJoint(`sc_${side}`),ac=api.getJoint(`ac_${side}`),gh=api.getJoint(`shoulder_${side}`),model=initShoulder(api,side),sign=model.sideSign,p=smooth((elevation-20)/160),upward=(51+3*(1-planeShare))*p,posterior=(16+5*planeShare)*p,external=(8+5*planeShare)*p,y=model.restPivotWorld.y+.018*p;let x=model.restPivotWorld.x+sign*.020*p,pivot=thoraxSurface(model,x,y),frame=buildScapulaQuaternion(model,pivot,upward,posterior,external,model.restOffset);if(!model.restOffset){model.restOffset=frame.qBase.clone().invert().multiply(model.restQuaternion.clone());frame=buildScapulaQuaternion(model,pivot,upward,posterior,external,model.restOffset)}setShoulderChainWorldPose(sc,ac,scap,model,pivot,frame.q);api.jointRoot.updateMatrixWorld(true);
