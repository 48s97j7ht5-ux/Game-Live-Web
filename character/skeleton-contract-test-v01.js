...
const mod=await import('./skeleton-v13.js?v=20260812-1718');
...
// Debug rig only visualizes the same joint coordinates. It is not part of runtime architecture.
...
function apply(){const v=read();mechanics.reset();const out=mechanics.setLegPose('L',v);syncDebug();show(out);const m=api.getRestMetrics();status.textContent=`OK · native dynamic · contract ${api.contractVersion} · thigh ${(m.segments.thigh_L*1000).toFixed(0)} mm · shin ${(m.segments.shin_L*1000).toFixed(0)} mm`;}
...
document.getElementById('resetMechanics').addEventListener('click',()=>{for(const f of Object.values(fields))f.value='0';mechanics.reset();syncDebug();show({hipFlexion:0,kneeFlexion:0,ankleFlexion:0});status.textContent='Сброшено · native dynamic skeleton';});
