...
/** Skeleton v1.6 — anatomically corrected geometry baseline. */
export const SKELETON_CONTRACT_VERSION=1;
export const SKELETON_VERSION='1.6';
...
function buildLeg(side){const g=side==='L'?-1:1,hipW=new THREE.Vector3(g*hx,hipY,.014),kneeW=new THREE.Vector3(g*hx*.90,kneeY,-.020),ankW=new THREE.Vector3(g*hx*.86,ankleY,-.048),hip=joint(`hip_${side}`,pelvis,worldLocal(pelvis,hipW)),knee=joint(`knee_${side}`,hip,worldLocal(hip,kneeW)),ankle=joint(`ankle_${side}`,knee,worldLocal(knee,ankW));node(hip,`hip_${side}_head`,new THREE.Vector3(),.026,MAT.joint);const fNeck=new THREE.Vector3(g*(hx+.028),hipY-.032,.008),GT=new THREE.Vector3(g*(hx+.058),hipY-.026,-.006),LT=new THREE.Vector3(g*(hx+.020),hipY-.074,.016),shaft=new THREE.Vector3(g*(hx+.034),hipY-.100,-.006),cm=new THREE.Vector3(g*hx*.90-g*.020,kneeY+.005,-.020),cl=new THREE.Vector3(g*hx*.90+g*.023,kneeY+.003,-.019);for(const [n,p,r] of [['fNeck',fNeck,.014],['GT',GT,.019],['LT',LT,.012],['fShaftTop',shaft,.015],['fCondMed',cm,.020],['fCondLat',cl,.021]])node(hip,n+side,worldLocal(hip,p),r,MAT.frame);for(const [n,a,b,r,m] of [['hip_fNeck',hipW,fNeck,.014,MAT.bone],['fNeck_shaft',fNeck,shaft,.016,MAT.bone],['fNeck_GT',fNeck,GT,.011,MAT.frame],['shaft_LT',shaft,LT,.010,MAT.frame],['femur_med',shaft,cm,.018,MAT.bone],['femur_lat',shaft,cl,.018,MAT.bone]])wb(hip,n+side,a,b,r,m);const tm=new THREE.Vector3(g*hx*.90-g*.019,kneeY-.030,-.018),tl=new THREE.Vector3(g*hx*.90+g*.021,kneeY-.031,-.017),tt=new THREE.Vector3(g*hx*.90,kneeY-.057,-.020),fh=new THREE.Vector3(g*hx*.90+g*.036,kneeY-.052,-.022),pat=new THREE.Vector3(g*hx*.90,kneeY-.005,.026),fa=new THREE.Vector3(g*(hx*.86+.032),ankleY+.004,-.050);for(const [n,p,r,m] of [['tPlatMed',tm,.017,MAT.frame],['tPlatLat',tl,.017,MAT.frame],['tShaftTop',tt,.012,MAT.frame],['fibHead',fh,.010,MAT.frame],['fibAn',fa,.009,MAT.frame]])node(knee,n+side,worldLocal(knee,p),r,m);node(hip,'patella'+side,worldLocal(hip,pat),.016,MAT.joint);for(const [n,a,b,r,m] of [['plateau',tm,tl,.010,MAT.frame],['tibia_med',tm,tt,.013,MAT.bone],['tibia_lat',tl,tt,.013,MAT.bone],['tibia_shaft',tt,ankW,.015,MAT.bone],['fibula',fh,fa,.007,MAT.frame],['patella_link',pat,kneeW,.006,MAT.cart]]){if(n==='patella_link')wb(hip,n+side,a,b,r,m);else wb(knee,n+side,a,b,r,m);}node(ankle,`ankle_${side}_marker`,new THREE.Vector3(),.019,MAT.joint);const L=A.foot/1.07,talus=new THREE.Vector3(ankW.x,ankleY-.018,-.040),subtalar=joint(`subtalar_${side}`,ankle,worldLocal(ankle,talus)),calc=new THREE.Vector3(ankW.x,.050,-L*.205),heel=new THREE.Vector3(ankW.x,.025,-L*.255),nav=new THREE.Vector3(ankW.x-g*.010,.060,L*.055),cunM=new THREE.Vector3(ankW.x-g*.020,.052,L*.205),cunI=new THREE.Vector3(ankW.x-g*.004,.051,L*.205),cunL=new THREE.Vector3(ankW.x+g*.012,.049,L*.198),cub=new THREE.Vector3(ankW.x+g*.030,.038,L*.175);for(const [n,p,r,m] of [['talus',talus,.015,MAT.joint],['calcaneus',calc,.017,MAT.frame],['heel',heel,.013,MAT.end],['navicular',nav,.010,MAT.frame],['cuneiform_medial',cunM,.010,MAT.frame],['cuneiform_intermediate',cunI,.009,MAT.frame],['cuneiform_lateral',cunL,.009,MAT.frame],['cuboid',cub,.010,MAT.frame]])node(subtalar,`${n}_${side}`,worldLocal(subtalar,p),r,m);for(const [n,a,b,r] of [['talus_calc',talus,calc,.010],['calc_heel',calc,heel,.011],['talus_nav',talus,nav,.008],['calc_cub',calc,cub,.008],['nav_cunM',nav,cunM,.007],['nav_cunI',nav,cunI,.006],['nav_cunL',nav,cunL,.006],['calc_nav',calc,nav,.006]])wb(subtalar,`${n}_${side}`,a,b,r,n==='calc_nav'?MAT.frame:MAT.bone);const offs=[-.030,-.015,0,.016,.031].map(v=>v*g),starts=[cunM,cunI,cunL,cub,cub],lens=[.55,.62,.64,.61,.57];for(let i=0;i<5;i++){
 const base=new THREE.Vector3(ankW.x+offs[i]*.55,i<3?.047:.035,L*.245),mh=new THREE.Vector3(ankW.x+offs[i],i===0?.032:.025,L*lens[i]),p1=new THREE.Vector3(ankW.x+offs[i]*1.04,.020,L*(lens[i]+(i===0?.105:.070))),p2=i===0?null:new THREE.Vector3(ankW.x+offs[i]*1.07,.018,L*(lens[i]+.125)),tip=new THREE.Vector3(ankW.x+offs[i]*1.08,.016,L*(lens[i]+(i===0?.205:.175)));
 wn(subtalar,`base_${side}_${i}`,...base.toArray(),.007,MAT.frame);wb(subtalar,`mt1_${side}_${i}`,starts[i],base,.006,MAT.frame);wb(subtalar,`mt2_${side}_${i}`,base,mh,i===0?.008:.006);
 const mtp=joint(`toe_mtp_${side}_${i}`,subtalar,worldLocal(subtalar,mh));node(mtp,`head_${side}_${i}`,new THREE.Vector3(),i===0?.009:.007,MAT.joint);
 if(i===0){const ip=joint(`toe_ip_${side}_0`,mtp,worldLocal(mtp,p1));node(ip,`toe1_${side}_0`,new THREE.Vector3(),.006,MAT.joint);link(mtp,`toe_prox_${side}_0`,new THREE.Vector3(),ip.position,.007);const tipLocal=worldLocal(ip,tip);node(ip,`tip_${side}_0`,tipLocal,.006,MAT.end);link(ip,`toe_dist_${side}_0`,new THREE.Vector3(),tipLocal,.006);}
 else{const pip=joint(`toe_pip_${side}_${i}`,mtp,worldLocal(mtp,p1));node(pip,`toe1_${side}_${i}`,new THREE.Vector3(),.0055,MAT.joint);link(mtp,`toe_prox_${side}_${i}`,new THREE.Vector3(),pip.position,.0055);const dip=joint(`toe_dip_${side}_${i}`,pip,worldLocal(pip,p2));node(dip,`toe2_${side}_${i}`,new THREE.Vector3(),.005,MAT.joint);link(pip,`toe_mid_${side}_${i}`,new THREE.Vector3(),dip.position,.0048);const tipLocal=worldLocal(dip,tip);node(dip,`tip_${side}_${i}`,tipLocal,.006,MAT.end);link(dip,`toe_dist_${side}_${i}`,new THREE.Vector3(),tipLocal,.0045);}
}}
buildLeg('L');buildLeg('R');

function buildHand(side){
 const g=side==='L'?-1:1,wrist=joints.get(`wrist_${side}`),grp=new THREE.Group();grp.name=`hand_${side}`;wrist.add(grp);
 const hs=A.hand/.227,prox=[],dist=[],lat=[.018,.006,-.007,-.019];
 for(let i=0;i<4;i++)prox.push(node(grp,`carpal_p_${side}_${i}`,new THREE.Vector3(g*lat[i],-.018*hs,i===0?.006:i===3?-.005:0),.0055,MAT.frame));
 for(let i=0;i<4;i++)dist.push(node(grp,`carpal_d_${side}_${i}`,new THREE.Vector3(g*lat[i]*1.08,-.040*hs,Math.abs(i-1.5)*.004),.0055,MAT.frame));
 for(let i=0;i<3;i++){link(grp,`cp_${side}_${i}`,prox[i].position,prox[i+1].position,.0028,MAT.frame);link(grp,`cd_${side}_${i}`,dist[i].position,dist[i+1].position,.0028,MAT.frame)}
 const mcX=[.030,.018,.004,-.011,-.026],mcLen=[.064,.086,.091,.085,.076],heads=[];
 for(let i=0;i<5;i++){const base=dist[Math.min(i,3)],hp=new THREE.Vector3(g*mcX[i],(-.040-mcLen[i])*hs,i===0?.020:Math.abs(i-2)*.003);link(grp,`metacarpal_${side}_${i}`,base.position,hp,i===0?.0048:.0042);heads.push(hp)}
 const segs=[[.043,.026,.021],[.048,.030,.023],[.044,.028,.022],[.036,.023,.019]];
 for(let f=1;f<5;f++){
  const mcp=joint(`finger_mcp_${side}_${f}`,grp,heads[f]);node(mcp,`metacarpal_head_${side}_${f}`,new THREE.Vector3(),.0058,MAT.joint);
  const dx=(2.2-f)*.003*g,pip=joint(`finger_pip_${side}_${f}`,mcp,new THREE.Vector3(dx,-segs[f-1][0]*hs,0));node(pip,`finger_${side}_${f}_0`,new THREE.Vector3(),.005,MAT.joint);link(mcp,`phalanx_${side}_${f}_0`,new THREE.Vector3(),pip.position,.0038);
  const dip=joint(`finger_dip_${side}_${f}`,pip,new THREE.Vector3(dx,-segs[f-1][1]*hs,0));node(dip,`finger_${side}_${f}_1`,new THREE.Vector3(),.005,MAT.joint);link(pip,`phalanx_${side}_${f}_1`,new THREE.Vector3(),dip.position,.0033);
  const tip=new THREE.Vector3(dx,-segs[f-1][2]*hs,.002);node(dip,`finger_${side}_${f}_2`,tip,.0045,MAT.end);link(dip,`phalanx_${side}_${f}_2`,new THREE.Vector3(),tip,.0031);
 }
 const thumbBase=dist[0].position.clone(),thumbCmc=joint(`thumb_cmc_${side}`,grp,thumbBase),thumbMcp=joint(`thumb_mcp_${side}`,thumbCmc,heads[0].clone().sub(thumbBase));
 node(thumbMcp,`metacarpal_head_${side}_0`,new THREE.Vector3(),.0065,MAT.joint);link(thumbCmc,`thumb_metacarpal_${side}`,new THREE.Vector3(),thumbMcp.position,.0048);
 const thumbIp=joint(`thumb_ip_${side}`,thumbMcp,new THREE.Vector3(g*.022,-.031*hs,.012));node(thumbIp,`thumb_ip_marker_${side}`,new THREE.Vector3(),.0055,MAT.joint);link(thumbMcp,`thumb_prox_${side}`,new THREE.Vector3(),thumbIp.position,.0042);
 const thumbTip=new THREE.Vector3(g*.014,-.026*hs,.006);node(thumbIp,`thumb_tip_${side}`,thumbTip,.005,MAT.end);link(thumbIp,`thumb_dist_${side}`,new THREE.Vector3(),thumbTip,.0038);
}
...
function getGeometryMetrics(){root.updateMatrixWorld(true);return{
 ...
 componentChecks:{sacrum:!!named.get('sacrumMid'),siPair:!!joints.get('si_L')&&!!joints.get('si_R'),costalCartilage:!!root.getObjectByName('costal_cartilage_r1L'),forearmRotation:!!joints.get('forearm_rotation_L')&&!!joints.get('forearm_rotation_R'),threeCuneiforms:!!named.get('cuneiform_medial_L')&&!!named.get('cuneiform_intermediate_L')&&!!named.get('cuneiform_lateral_L'),mandible:!!joints.get('jaw'),articulatedDigits:!!joints.get('finger_mcp_L_2')&&!!joints.get('toe_mtp_L_1')}
}}
...
function rebuildRestPose({stature:newStature=stature}={}){if(Math.abs(newStature-stature)>.000001)throw new Error('Skeleton v1.6: stature rebuild after geometry validation');resetPose();return getRestMetrics()}
...
metrics.innerHTML=`<h3>Диагностика v1.6</h3><div class="row"><span>Рост</span><span>1750 мм</span></div><div class="row"><span>Architecture</span><span>standalone dynamic</span></div><div class="row"><span>Contract</span><span>v1</span></div><div class="row"><span>Limbs</span><span>native L/R</span></div><div class="row"><span>Spine</span><span>25 native vertebral joints</span></div><div class="row"><span>Shoulders</span><span>SC → AC → scapula/ST → GH</span></div>`;
const title=document.querySelector('.info .title');if(title)title.textContent='Skeleton v1.6';const sub=document.querySelector('.info .sub');if(sub)sub.innerHTML='anatomically corrected geometry · contract v1<br>pelvic ring · closed thorax · complete hands and feet';
