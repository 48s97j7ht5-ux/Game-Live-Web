...
document.querySelectorAll('.speed button').forEach(button=>button.onclick=()=>{document.querySelectorAll('.speed button').forEach(item=>item.classList.remove('active'));button.classList.add('active');speed=+button.dataset.speed;run()});
...
document.querySelector('#look').onclick=()=>{document.querySelector('#worldText').textContent='У подъезда пусто. В нескольких окнах горит свет, где-то наверху хлопнула дверь.';minutes++;render()};
document.querySelector('#enter').onclick=()=>{document.querySelector('#worldText').textContent='Ты заходишь в подъезд. Пахнет прохладным бетоном и утренним кофе из одной из квартир.';minutes+=2;render();say('Локация изменена')};
document.querySelectorAll('.ios-app').forEach(button=>button.onclick=()=>say(button.id==='navigator'?'Навигатор: пока открыт только твой дом':button.dataset.name+' — приложение появится позже'));
...
document.querySelector('#layerUp').onclick=()=>{clothingLayerIndex=Math.min(clothingLayers.length-1,clothingLayerIndex+1);renderLayer();say(clothingLayers[clothingLayerIndex].name)};
document.querySelector('#layerDown').onclick=()=>{clothingLayerIndex=Math.max(0,clothingLayerIndex-1);renderLayer();say(clothingLayers[clothingLayerIndex].name)};

renderLayer();render();run();