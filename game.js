...
document.querySelectorAll('.ios-app').forEach(button=>{
  button.addEventListener('click',()=>say(button.id==='navigator'?'Навигатор: пока открыт только твой дом':button.dataset.name+' — приложение появится позже'));
});

meTabs.forEach((button,index)=>{
  button.addEventListener('click',()=>{
    meTabs.forEach(item=>item.classList.remove('active'));
    button.classList.add('active');
