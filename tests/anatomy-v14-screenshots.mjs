...
const out='anatomy-test-output/v14-shoulder';await fs.rm(out,{recursive:true,force:true});await fs.mkdir(out,{recursive:true});
const browser=await chromium.launch({headless:true});
for(const view of [{name:'front',pos:[0,1.15,4.7]},{name:'side',pos:[4.7,1.15,0]},{name:'back',pos:[0,1.15,-4.7]},{name:'threequarter',pos:[3.2,1.35,3.2]}]){
...
await browser.close();console.log('v1.4 shoulder regression captured');
